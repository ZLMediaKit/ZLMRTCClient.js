...
function maybeSetAudioSendBitRate(sdp, params) {
  ...
  debug.log('Prefer audio send bitrate: ' + params.audioSendBitrate);
...
function maybeSetAudioReceiveBitRate(sdp, params) {
  ...
  debug.log('Prefer audio receive bitrate: ' + params.audioRecvBitrate);
...
function maybeSetVideoSendBitRate(sdp, params) {
  ...
  debug.log('Prefer video send bitrate: ' + params.videoSendBitrate);
...
function maybeSetVideoReceiveBitRate(sdp, params) {
  ...
  debug.log('Prefer video receive bitrate: ' + params.videoRecvBitrate);
...
function preferBitRate(sdp, bitrate, mediaType) {
  ...
  if (mLineIndex === null) {
    debug.log('Failed to add bandwidth line to sdp, as no m-line found');
    return sdp;
  }
  ...
  const cLineIndex = findLineInRange(sdpLines, mLineIndex + 1,
      nextMLineIndex, 'c=');
  if (cLineIndex === null) {
    debug.log('Failed to add bandwidth line to sdp, as no c-line found');
    return sdp;
  }
...
function maybeSetVideoSendInitialBitRate(sdp, params) {
  ...
  if (bitrate) {
    if (initialBitrate > bitrate) {
      debug.log('Clamping initial bitrate to max bitrate of ' + bitrate + ' kbps.');
  ...
  if (mLineIndex === null) {
    debug.log('Failed to find video m-line');
    return sdp;
  }
...
function maybePreferCodec(sdp, type, dir, codec) {
  ...
  if (!codec) {
    debug.log('No preference on ' + str + '.');
    return sdp;
  }

  debug.log('Prefer ' + str + ': ' + codec);
